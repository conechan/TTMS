...
class Servers extends Controller {
    ...
    function index($offset = 0)
    {
        ...
        $this->table->set_heading('Host name', 'Status', 'IP', 'Type', 'System', 'Login<br />name', 'Login<br />password', 'root<br />password','Actions', 'Manage');
        foreach ($server as $s)
        {
            $action_list = '';
            $system_list = '';
            ...
            foreach ($s->actions as $a)
            {
                ...
                $this->table->add_row($s->name, "<span class=\"pingstat\" ipp=\"$s->ip\"></span>", $s->ip, $s->type->name,
                    $system_list, $s->login_name, $s->login_pwd, $s->root_pwd,
                    $action_list,
                    anchor('servers/view/'.$s->id,'view', 'class="view"').'<br />'.
                    anchor('servers/edit/'.$s->id,'edit', 'class="edit"').'<br />'.
    ...
    function find($type = '', $system = '')
    {
        $this->load->library('login_manager');
        
		$type = $this->uri->segment(3);
        //$system = $this->uri->segment(4);
        ...
        $t->get_by_name("$type");
        //$sys->get_by_name("$system");
        $server->get_by_related($t);
        //$server->get_by_related($sys);
        ...
        $this->table->set_template($tmpl);
        $this->table->set_heading('Host name', 'Status', 'IP', 'Type', 'System', 'Actions');
        foreach ($server as $s)
        {
            $action_list = '';
            $system_list = '';
