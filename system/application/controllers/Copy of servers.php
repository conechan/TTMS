...
class Servers extends Controller {
    ...
    function index($offset = 0)
    {
        ...
        foreach ($server as $s)
        {
            ...
            {
                $action_list = $action_list.'<li>'.anchor_popup('servers/execute/'.$s->ip.'/'.$a->command, $a->name).'</li>';
            }
                $this->table->add_row($s->name, $s->ip, $s->type->name,
                    $s->system->name, $s->login_name, $s->login_pwd, $s->root_pwd,
                    $action_list,
    ...
    function find($type = '')
    {
        ...
        foreach ($server as $s)
        {
            ...
            foreach ($s->actions as $a)
            {
                $action_list = $action_list.'<li>'.anchor_popup('servers/execute/'.$s->ip.'/'.$a->command.'/'.$s->name, $a->name).'</li>';
            }
                $this->table->add_row($s->name, $s->ip, $s->type->name,
                    $s->system->name,
                    $action_list);
